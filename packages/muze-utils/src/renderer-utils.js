...
const
     ...
     * @return {Selection} Instance of d3 selection.
     */
    selectAllElements = element => selectAll(element),
    /**
    ...
    */
     ...
     */
    makeElement = (parent, elemType, data, selector, callbacks = {}, keyFn) => {
    ...
    */
     ...
     */
    getSVGGroup = (htmlContainer, className, width, height) => {
        let sel;
     ...
     */
    appendElement = (element1, element2) => {
     ...
     */
    setAttrs = (element, attrs, className) => {
    ...
    createElements = (params) => {
        ...
        selectionMerge = sel.enter().append(params.append).merge(sel).each(function (d, i) {
     ...
     */
    clipElement = (container, measurement, id) => {
        const clipPathElement = makeElement(select(container), 'clipPath', [1], `#${id}`),
            clipPathRect = makeElement(clipPathElement, 'rect', [1]);
     ...
     */
    getElementsByClassName = (node, className) => select(node).selectAll(className).nodes(),
    /**
     ...
     */
    createElement = (tag, mount) => select(mount).append(tag).node(),
    /**
     ...
     */
    createCaption = (container, config) => {
        let caption = select(container).selectAll(config.className).data([1]),
            captionEl = caption.enter().append('p').merge(caption).text(config.text);
