...
(function () {
    ...
    layerFactory.composeLayers('lineWithLabels', [{
        ...
        positioner: (points, store, layerInst) => {
            ...
            let originField = schema.findIndex(d => d.name === colorField);
            for (let i = 0; i < points.length; i++) {
                let data = points[i]._data;
                originValue = data.filter((d, i) => i === originField);
                let text = points[i].text;
                let size = smartLabel.getOriSize(text);
                let textHeight = size.height;
                let textWidth = size.width;
                let linePoints = store.layers.simpleline.getPointsFromIdentifiers([[colorField], originValue]);
                if (linePoints) {
                    let len = linePoints.length - 1;
                    let prevPoint = linePoints[len - 1];
                    let currentPoint = linePoints[len];
                    if (prevPoint.y < currentPoint.y) {
                        points[i].update.y = points[i].update.y + textHeight;
                    }
                    if (points[i].update.x + textWidth > unitWidth) {
                        points[i].update.x -= ((points[i].update.x + textWidth) - unitWidth) / 2;
                    }
                }
            }
            return points;
